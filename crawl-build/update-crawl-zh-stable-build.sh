...
source $DGL_CONF_HOME/crawl-zh-git.conf
GAME=crawl-$VERSION-zh

export DESTDIR=$CRAWL_BASEDIR
BRANCH=stone_soup-$VERSION-zh
if [[ $VERSION != [0-9]* ]]; then
    BRANCH=$VERSION
fi
...
REVISION="$2"
./update-public-repository-zh.sh $BRANCH "$REVISION"
...
say-do sudo -H $DGL_CHROOT/sbin/install-zh-stable.sh "$VERSION"
